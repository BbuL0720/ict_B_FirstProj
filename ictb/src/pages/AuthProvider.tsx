...
interface AuthContextProps {
  ...
  checkLogin: () => void;
  isLoggedIn: boolean;
  login: (mid: string, password: string) => Promise<'success' | 'fail' | 'error'>;
  logout: () => void;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (mid: string, password: string): Promise<'success' | 'fail' | 'error'> => {
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_BACK_END_URL}/login/dologin`,
        { mid, password }, { withCredentials: true }
      );
      if (res.data === 'success') {
